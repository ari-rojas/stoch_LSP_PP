...
class data_mgmt():
    ...
    def import_data(experiment, page, st, container):
        ...
        for tup in container:
            ...
            elif page == 2: prefix = "TP_service_level"; beta,b = tup; a = 0.4; epsilon = 1
...
class charts():
    ...
    def generate_plot_specifics(page, view_bool, mode, last, xaxis_bool, cont_0, bn1):
        ...
        if view_bool:
            ...
            if not xaxis_bool:
                ...
                x_ax_title = r"$\alpha_{n-1}-\alpha_0$" if page == 1 else r"$\alpha_{n-1}-\beta$"
    ...
    @staticmethod
    def plot_service_level_analysis(cont_0, bn1, reps, T, S, G, f, c, C, setup, holding, production, total, total_sl, waste_prod, waste_dem, view_bool, hover_bool, st, specifics, xaxis_bool):
        ...
        for y in y_axis_container:
            ...
            fig.add_trace(go.Scatter(x=x_axis_container[y], y=[np.average([total_sl[comb[x]][rep][0] for rep in reps]) for x in x_axis_container[y]], customdata=x_hover_text[y], mode="lines+markers", hovertemplate=hover_text+": %{customdata:.1%} <br>Fresh produce fill rate = %{y:,.2%}", marker={"color":cols[ix]}, name=f"{y:.1%}", legendgroup=f"{y:.1%}", showlegend=False), row=2, col=1)
            fig.add_trace(go.Scatter(x=x_axis_container[y], y=[np.average([total_sl[comb[x]][rep][G[-1]] for rep in reps]) for x in x_axis_container[y]], customdata=x_hover_text[y], mode="lines+markers", hovertemplate=hover_text+": %{customdata:.1%} <br>Overall fill rate = %{y:,.2%}", marker={"color":cols[ix]}, name=f"{y:.1%}" , legendgroup=f"{y:.1%}", showlegend=False), row=2, col=2)
        ...
        fig.update_yaxes(showline=True, linewidth=1, linecolor="black", tickvals=fresh_prod_sl["vals"], ticks=fresh_prod_sl["ticks"], ticktext=fresh_prod_sl["ticktext"], row=2, col=1, tickfont={"color":"black"})
    ...
    @staticmethod
    def gen_ind_sl_analysis(data, page, container, cont_0, bn1, reps, st, view_bool, hover_bool, specifics, xaxis_bool):
        ...
        fig = charts.plot_service_level_analysis(cont_0, bn1, reps, T, S, G, f, c, C, setup, holding, production, total, total_sl, waste_prod, waste_dem, view_bool, hover_bool, st, specifics, xaxis_bool)

        return fig
    
    @staticmethod
    def gen_comb_sl_analysis(data, page, alpha, cont_0, bn1, reps, view_bool, hover_bool, plot, share_y, specifics, xaxis_bool):

        setup, holding, production, total, period_sl, total_sl, waste_prod, waste_dem = dict(), dict(), dict(), dict(), dict(), dict(), dict(), dict()

        (T, S, G, n, h, c, f, C), metrics = data_mgmt.import_data(data, page, "stat", alpha)
        (setup[1], holding[1], production[1], total[1], period_sl[1], total_sl[1], waste_prod[1], waste_dem[1]) = data_mgmt.process_data(S, metrics, reps, alpha); del metrics

        (T, S, G, n, h, c, f, C), metrics = data_mgmt.import_data(data, page, "dyn", alpha)
        (setup[2], holding[2], production[2], total[2], period_sl[2], total_sl[2], waste_prod[2], waste_dem[2]) = data_mgmt.process_data(S, metrics, reps, alpha); del metrics
        ...
        for y in y_axis_container:
            ...
            for col in [1,2]:
                if plot == "tot_cost":
                    ...
                    fig.add_trace(go.Scatter(x=x_axis_container[y], y=[np.average([setup[col][comb[x]][rep]/(f*len(T)) for rep in reps]) for x in x_axis_container[y]], customdata=x_hover_text[y], mode="lines+markers", hovertemplate=hover_text+": %{customdata:.1%} <br>Setups = %{y:,.2%}", marker={"color":cols[ix]}, name=f"{y:.1%}" , legendgroup=f"{y:.1%}", showlegend=True if col == 2 else False), row=1, col=col)
                elif plot == "lot_size":    
                    fig.add_trace(go.Scatter(x=x_axis_container[y], y=[np.average([production[col][comb[x]][rep]*f/(C*setup[col][comb[x]][rep]*c) for rep in reps]) for x in x_axis_container[y]], customdata=x_hover_text[y], mode="lines+markers", hovertemplate=hover_text+": %{customdata:.1%} <br>Avg. Utilization = %{y:,.2%}", marker={"color":cols[ix]}, name=f"{y:.1%}", legendgroup=f"{y:.1%}", showlegend=True if col == 2 else False), row=1, col=col)
                elif plot == "prod_cost":    
                    fig.add_trace(go.Scatter(x=x_axis_container[y], y=[np.average([production[col][comb[x]][rep] for rep in reps]) for x in x_axis_container[y]], customdata=x_hover_text[y], mode="lines+markers", hovertemplate=hover_text+": %{customdata:.1%} <br>Prod. Cost = %{y:,.0f}", marker={"color":cols[ix]}, name=f"{y:.1%}", legendgroup=f"{y:.1%}", showlegend=True if col == 2 else False), row=1, col=col)
                elif plot == "hold_cost":    
                    fig.add_trace(go.Scatter(x=x_axis_container[y], y=[np.average([holding[col][comb[x]][rep] for rep in reps]) for x in x_axis_container[y]], customdata=x_hover_text[y], mode="lines+markers", hovertemplate=hover_text+": %{customdata:.1%} <br>Exp. Hold. Cost = %{y:,.0f}", marker={"color":cols[ix]}, name=f"{y:.1%}", legendgroup=f"{y:.1%}", showlegend=True if col == 2 else False), row=1, col=col)
                elif plot == "fresh_fr":    
                    fig.add_trace(go.Scatter(x=x_axis_container[y], y=[np.average([total_sl[col][comb[x]][rep][0] for rep in reps]) for x in x_axis_container[y]], customdata=x_hover_text[y], mode="lines+markers", hovertemplate=hover_text+": %{customdata:.1%} <br>Fresh produce fill rate = %{y:,.2%}", marker={"color":cols[ix]}, name=f"{y:.1%}", legendgroup=f"{y:.1%}", showlegend=True if col == 2 else False), row=1, col=col)
                elif plot == "over_fr":
                    fig.add_trace(go.Scatter(x=x_axis_container[y], y=[np.average([total_sl[col][comb[x]][rep][G[-1]] for rep in reps]) for x in x_axis_container[y]], customdata=x_hover_text[y], mode="lines+markers", hovertemplate=hover_text+": %{customdata:.1%} <br>Overall fill rate = %{y:,.2%}", marker={"color":cols[ix]}, name=f"{y:.1%}" , legendgroup=f"{y:.1%}", showlegend=True if col == 2 else False), row=1, col=col)
                elif plot == "w_prod":    
                    fig.add_trace(go.Scatter(x=x_axis_container[y], y=[np.average([waste_prod[col][comb[x]][rep] for rep in reps]) for x in x_axis_container[y]], customdata=x_hover_text[y], mode="lines+markers", hovertemplate=hover_text+": %{customdata:.1%} <br>Waste (/prod) = %{y:.2%}", marker={"color":cols[ix]}, name=f"{y:.1%}" , legendgroup=f"{y:.1%}", showlegend=True if col == 2 else False), row=1, col=col)
                elif plot == "w_dem":    
                    fig.add_trace(go.Scatter(x=x_axis_container[y], y=[np.average([waste_dem[col][comb[x]][rep] for rep in reps]) for x in x_axis_container[y]], customdata=x_hover_text[y], mode="lines+markers", hovertemplate=hover_text+": %{customdata:.1%} <br>Waste (/dem) = %{y:.2%}", marker={"color":cols[ix]}, name=f"{y:.1%}" , legendgroup=f"{y:.1%}", showlegend=True if col == 2 else False), row=1, col=col)
            ix += 1
