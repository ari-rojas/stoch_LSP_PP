                    ...
                    html.Div([
                        ...
                        html.Div([
                            ...
                            chart_radio := dcc.RadioItems([
                                ...
                                ], value="tot_cost", className="w-100 px-3"),
                            ...
                            ], style={"margin":"15px"}, className="vstack gap-4 border rounded w-25 h-50 bg-white justify-content-center"),
        ...
        side_settings := dbc.Offcanvas([html.Div([
                ...
                dbc.Card([html.Div([
                    ...
                    html.Div([
                        ...
                        relative_last := dcc.Dropdown(options=[{"label":html.Div(f"{a:.0%}", style={"color":"black"}), "value":a} for a in a0 if a <= 0.7], value=0.4, style={"color":"black", "width":"90px"}, disabled=True)
...
def update_layout(view_bool, hover_bool, range_0, range_n1, m, sl_n, plot_radio, scale_y, np_mode, np_last, xaxis_bool, n_download, rel_mode, rel_last):
    ...
    if rel_mode == "abs":
        fig1 = stoch_LSP_PP.charts.gen_ind_sl_analysis(data, page, alpha, b0, bn1, reps, st, sl_type, view_bool, hover_bool, specifics, xaxis_bool)
        fig2 = stoch_LSP_PP.charts.gen_comb_sl_analysis(data, page, alpha, b0, bn1, reps, sl_type, view_bool, hover_bool, plot_radio, scale_y, specifics, xaxis_bool, download)
    else:
        fig1 = stoch_LSP_PP.charts.gen_ind_sl_analysis_relative(data, page, alpha, b0, bn1, reps, st, sl_type, view_bool, hover_bool, specifics, xaxis_bool, rel_last)
        fig2 = stoch_LSP_PP.charts.gen_comb_sl_analysis_relative(data, page, alpha, b0, bn1, reps, sl_type, view_bool, hover_bool, plot_radio, scale_y, specifics, xaxis_bool, rel_last, download)
...
def alpha0_disabling(view_bool):
    ...
    if view_bool:
        ...
        rel_drop_options = [{"label":html.Div(f"{a:.0%}", style={"color":"black"}), "value":a} for a in a0 if a <= 0.7]
        ...
        rel_drop_options = [{"label":html.Div(f"{a:.0%}", style={"color":"silver"}), "value":a} for a in a0 if a <= 0.7]
